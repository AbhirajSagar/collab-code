  ...
  return (
    <>
      <div className="w-full h-10 mt-15 flex">
        {Tabs.map((tab, idx) => <h2 key={idx} onClick={() => setCurrentTab(tab)} className={`w-fit mx-3 cursor-pointer ${tab === currentTab ? 'text-orange-400 underline' : 'text-muted'}`}>{tab.name}</h2>)}
      </div>
      <div className="dark:bg-background-muted-dark bg-white h-max w-1/2 rounded-md">
        {currentTab.content}
      </div>
    </>
  );
